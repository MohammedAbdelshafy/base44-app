...
async def main():
    ...
    # Get MuslimsClipping campaigns specifically
    rows = await conn.fetch("""
        SELECT id, title, status, platform_name, payout_per_1k_views, 
               ...
               due_at, created_at
        FROM campaigns 
        WHERE platform_name ILIKE '%muslim%' OR title ILIKE '%islamic%' OR title ILIKE '%halal%'
        ORDER BY created_at DESC
    """)
    
    print(f"MuslimsClipping campaigns: {len(rows)}")
    print("=" * 80)
    ...
    for row in rows:
        ...
        if r['intelligence_notes']:
            print(f"Intelligence: {r['intelligence_notes']}")
